...
const $imageMemoModalImageCardContainer = document.querySelector(
    '.image-memo .image-card-container'
); // 이미지 카드 컨테이너
const $imageMemoModalCloseBtn = document.querySelector(
    '.image-memo .close-btn'
); // 닫기 버튼
const $imageMemoModalViewBtns = document.querySelectorAll(
    '.image-memo .view-btn'
); // 보기 버튼들
const $imageMemoModalDeleteBtns = document.querySelectorAll(
    '.image-memo .delete-btn'
); // 삭제 버튼들
const $imageMemoModalImageUpload = document.getElementById('image-upload'); // 이미지 업로드 input
const MAXSIZE = 1024 * 1024; // 1MB
const $imageMemoModalUploadBtn = document.querySelector(
    '.image-memo .upload-btn'
); // 업로드 버튼
// 이미지 크게 보기 모달
const $imageMemoViewModal = document.querySelector('.image-memo-view'); // 이미지 크게 보기 모달
const $imageMemoViewModalCloseBtn = document.querySelector(
    '.image-memo-view .close-btn'
); // 닫기 버튼
const $imageMemoViewModalImage = document.querySelector(
    '.image-memo-view .content-image img'
); // 이미지 태그
const $imageMemoViewModalUpdateBtn = document.querySelector(
    '.image-memo-view .update-btn'
); // [수정하기] 버튼
// 메모 수정 모달
const $memoUpdateModal = document.querySelector('.memo-update'); // 메모 수정 모달
const $memoUpdateModalCloseBtn = document.querySelector(
    '.memo-update .close-btn'
); // 닫기 버튼
...
const $memoUpdateModalSaveBtn = document.querySelector(
    '.memo-update .save-btn'
); // 저장 버튼
...
const uploadImage = function (image) {
    ...
    reader.onload = (image) => {
        ...
        newImageCard
            ...
            .addEventListener('click', viewImageInModal); // 새로운 [보기] 버튼에 이벤트 리스너 추가
        newImageCard
            .querySelector('.delete-btn')
            .addEventListener('click', deleteImageCard); // 새로운 [삭제] 버튼에 이벤트 리스너 추가
